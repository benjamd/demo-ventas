...
@Component("/ventas/presupuesto/exportar.xlsx")
public class VentasPresupuestoMensualXlsxView extends AbstractXlsxView {

	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		response.setHeader("Content-Disposition", "attachment; filename=\"listado_presupuestos_de_venta.xlsx\" ");
		response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		@SuppressWarnings("unchecked")
		List<VentasPresupuesto> ordenesentrega =   (List<VentasPresupuesto>) model.get("presupuestos");
		Date fechainicio =   (Date) model.get("fechainicio");
		Date fechafin =  (Date)  model.get("fechafin");
		Sheet sheet = workbook.createSheet("Presupuestos de Venta");
		...
		tbodydateStyle.setDataFormat(createHelper.createDataFormat().getFormat("dd/mm/yyyy"));
		reporte.createCell(0).setCellValue("Reporte de ventas - Presupuestos de Venta");
    	...
    	Cell cell = null;
		for(int i = 0; i <   ordenesentrega.size() ; i++) {
			//FECHA
		...
		rownumber = 6;
		for(VentasPresupuesto ordenentrega: ordenesentrega) {
		  fila = sheet.getRow(rownumber ++);
		  cell = fila.getCell(0); 
		  cell.setCellValue(ordenentrega.getFecha());
		  cell = fila.getCell(1);  
		  cell.setCellValue(ordenentrega.getCliente().getRazonsocial());
		  cell = fila.getCell(2);  
		  cell.setCellValue(ordenentrega.getCliente().getCuit());
		  cell = fila.getCell(3); 
		  cell.setCellValue(ordenentrega.getPrefijo()+"-"+ df.format(ordenentrega.getNumero()));
		  cell = fila.getCell(4); 
		  cell.setCellValue(ordenentrega.getImporte().doubleValue());
		  totalNeto = totalNeto + ordenentrega.getImporte().doubleValue();
	   }
