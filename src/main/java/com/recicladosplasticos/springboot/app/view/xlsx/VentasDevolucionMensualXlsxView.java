...
@Component("/ventas/devolucion/exportar.xlsx")
public class VentasDevolucionMensualXlsxView extends AbstractXlsxView{

	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		 response.setHeader("Content-Disposition", "attachment; filename=\"listado_devoluciones.xlsx\" ");
		...
		List<VentasDevolucion> ventasDevoluciones =   (List<VentasDevolucion>) model.get("devoluciones");
		...
		reporte.createCell(0).setCellValue("Reporte de ventas - Devoluciones");
		...
		for(int i = 0; i <   ventasDevoluciones.size() ; i++) {
		...
		rownumber = 6;
		for(VentasDevolucion ventasDevolucion: ventasDevoluciones) {
		  
		  fila = sheet.getRow(rownumber ++);
		  
		  cell = fila.getCell(0); 
		  cell.setCellValue(ventasDevolucion.getFecha());
		  
		  cell = fila.getCell(1);  
		  cell.setCellValue(ventasDevolucion.getCliente().getRazonsocial());
		  
		  cell = fila.getCell(2);  
		  cell.setCellValue(ventasDevolucion.getCliente().getCuit());
		   
		  cell = fila.getCell(3); 
		  cell.setCellValue(ventasDevolucion.getPrefijo()+"-"+ df.format(ventasDevolucion.getNumero()));
		 
		  cell = fila.getCell(4); 
		  cell.setCellValue(ventasDevolucion.getLineas().doubleValue());

		  cell = fila.getCell(5); 
		  cell.setCellValue(ventasDevolucion.getCantidadtotal().doubleValue());

		  totalLineas += ventasDevolucion.getLineas(); 
		  totalCantidad += ventasDevolucion.getCantidadtotal().doubleValue();
