...
@Component("/ventas/debito/exportar.xlsx")
public class VentasDebitoMensualXlsxView  extends AbstractXlsxView {

	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		 
		response.setHeader("Content-Disposition", "attachment; filename=\"listado_ventas_debitos.xlsx\" ");
		 response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		 
	
		
		 @SuppressWarnings("unchecked")
		List<VentasNotaDeDebito> debitos =   (List<VentasNotaDeDebito>) model.get("debitos");
		
		 Date fechainicio =   (Date) model.get("fechainicio");
		 Date fechafin =  (Date)  model.get("fechafin");
			
		Sheet sheet = workbook.createSheet("Notas de Débito");
		...
		reporte.createCell(0).setCellValue("Reporte de ventas - Notas de Débito");
		...
		rownumber = 6;
		for(VentasNotaDeDebito debito: debitos) {
		  
		  fila = sheet.getRow(rownumber ++);
		  
		  cell = fila.getCell(0); 
		  cell.setCellValue(debito.getFecha());
		  
		  cell = fila.getCell(1);  
		  cell.setCellValue(debito.getCliente().getRazonsocial());
		  
		  cell = fila.getCell(2);  
		  cell.setCellValue(debito.getCliente().getCuit());
		   
		  cell = fila.getCell(3); 
		  cell.setCellValue(debito.getPrefijo()+"-"+ df.format(debito.getNumero()));
		 
		  cell = fila.getCell(4); 
		  cell.setCellValue(debito.getImporte().doubleValue());
		  
		  cell = fila.getCell(5); 
		  cell.setCellValue(debito.getImporteiva().doubleValue());

		  cell = fila.getCell(6); 
		  cell.setCellValue(debito.getImportetotal().doubleValue());
		  
		  totalNeto = totalNeto + debito.getImporte().doubleValue();
		  totalIva = totalIva + debito.getImporteiva().doubleValue();
		  totalVentas = totalVentas + debito.getImportetotal().doubleValue();
