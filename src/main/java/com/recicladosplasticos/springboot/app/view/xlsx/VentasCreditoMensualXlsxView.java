...
@Component("/ventas/credito/exportar.xlsx")
public class VentasCreditoMensualXlsxView extends AbstractXlsxView  {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"listado_ventas_creditos.xlsx\" ");
		 response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		 
	
		
		 @SuppressWarnings("unchecked")
		List<VentasNotaDeCredito> creditos =   (List<VentasNotaDeCredito>) model.get("creditos");
		
		 Date fechainicio =   (Date) model.get("fechainicio");
		 Date fechafin =  (Date)  model.get("fechafin");
			
		Sheet sheet = workbook.createSheet("Notas de Crédito");
		...
		//FORMATO CUERPO CLIENTE - NRO. NOTA DE CREDITO
		...
		reporte.createCell(0).setCellValue("Reporte de ventas - Notas de Crédito");
		...
		rownumber = 6;
		for(VentasNotaDeCredito credito: creditos) {
		  
		  fila = sheet.getRow(rownumber ++);
		  
		  cell = fila.getCell(0); 
		  cell.setCellValue(credito.getFecha());
		  
		  cell = fila.getCell(1);  
		  cell.setCellValue(credito.getCliente().getRazonsocial());
		  
		  cell = fila.getCell(2);  
		  cell.setCellValue(credito.getCliente().getCuit());
		   
		  cell = fila.getCell(3); 
		  cell.setCellValue(credito.getPrefijo()+"-"+ df.format(credito.getNumero()));
		 
		  cell = fila.getCell(4); 
		  cell.setCellValue(credito.getImporte().doubleValue());
		  
		  cell = fila.getCell(5); 
		  cell.setCellValue(credito.getImporteiva().doubleValue());

		  cell = fila.getCell(6); 
		  cell.setCellValue(credito.getImportetotal().doubleValue());
		  
		  totalNeto = totalNeto + credito.getImporte().doubleValue();
		  totalIva = totalIva + credito.getImporteiva().doubleValue();
		  totalVentas = totalVentas + credito.getImportetotal().doubleValue();
