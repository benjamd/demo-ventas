...
@Entity
@Table(name = "ventas_facturas_items")
public class ItemVentasFactura implements Serializable  {
	...
	public BigDecimal calcularImporte() {
		return this.getCantidad().multiply(this.getPrecio());
	}
	...
	public Double calcularImporteNeto() {
		...
		Double iNeto =  this.getCantidad() * (this.getPrecio() *( 1 - (this.getDescuento() /100)));
		
		return round(iNeto,2);
	}
 */
